...
AWS.config.update({
     accessKeyId: 'AKIA44U26FALAXIDBKB5', // Replace with your access key
     secretAccessKey: 'ZPQZAMVtJoqusYMRHJCR5Z55kJF7/9gcd6yESsOo', // Replace with your secret key
...
function allowCameraAccess(e) {
     ...
     navigator.mediaDevices.getUserMedia({
          ...
          .then(function (stream) {
               ...
               document.getElementById('title').innerText = 'Camera Access Granted';
               e.remove();
          })
          .catch(function (err) {
...
async function detectFace() {
     ...
     try {
          ...
          const byteArray = new Uint8Array(atob(imageBytes).split('').map(char => char.charCodeAt(0)));
...
function handleFaceDetection(data, blob, token, userId) {
     ...
     axios('/api/face/upload', {
          ...
          headers: {
               ...
               'mustard_seed_charity_session': getCookieValue('mustard_seed_charity_session'),
               'X-CSRF-TOKEN': getCookieValue('XSRF-TOKEN')
          },
          data: formData
     })
          .then(data => {
               const message = data.data.success ? 'Face detected' : 'Face not detected';
